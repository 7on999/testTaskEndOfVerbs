...
const conjugate = (infinitive, pronoun) => {

    let secondСonjugation = false;
    let reflexiveVerb = false;

    const vowelsArr = ['а', 'у', 'о', 'и', 'э', 'ы', 'я', 'ю', 'е','ё']

    const exceptionsOfSecondСonjugation = [
    'гнать', 'держать', 'терпеть', 'обидеть', 
    'слышать', 'видеть', 'ненавидеть',  
    'смотреть', 'дышать', 'зависеть', 'вертеть'
]

    const infinitiveArr = infinitive.toLowerCase().split('');
    const lengthInfinitive = infinitiveArr.length;
